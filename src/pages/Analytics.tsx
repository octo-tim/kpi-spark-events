              ...
              <tbody>
                {displayEvents.length > 0 ? (
                  displayEvents.map((event) => {
                     const achievementRate = ((event.actual_contracts / event.target_contracts) * 100).toFixed(1)
                     return (
                       <tr key={event.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                         ...
                         <td className="p-3">
                           <div className="text-sm">
                             ...
                             <div className="text-muted-foreground">~ {event.end_date}</div>
                           </div>
                         </td>
                         <td className="p-3 text-sm">{event.partner}</td>
                         <td className="p-3 text-right text-sm">{event.target_contracts}건</td>
                         <td className="p-3 text-right text-sm font-medium">{event.actual_contracts}건</td>
                         ...
                         <td className="p-3 text-right text-sm">{event.actual_sqm.toLocaleString()}장</td>
                         <td className="p-3 text-right text-sm">{(event.total_cost / 1000000).toFixed(1)}M원</td>
                         <td className="p-3 text-right">
                           <Badge 
                             className={`text-xs ${
                               event.efficiency >= 85 ? "bg-success text-success-foreground" :
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div className="text-center">
                <div className="font-semibold text-lg">
                  {displayEvents.reduce((sum, event) => sum + event.actual_contracts, 0)}
                </div>
                <div className="text-muted-foreground">총 계약건수</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-lg">
                  {displayEvents.reduce((sum, event) => sum + event.actual_sqm, 0).toLocaleString()}
                </div>
                <div className="text-muted-foreground">총 계약장수</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-lg">
                  {(displayEvents.reduce((sum, event) => sum + event.total_cost, 0) / 1000000).toFixed(1)}M
                </div>
